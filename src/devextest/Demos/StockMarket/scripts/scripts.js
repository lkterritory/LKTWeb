...
(function (StockMarket, $) {
  StockMarket.loadData = function (options) {
    return $.ajax({
      url:
        "https://js.devexpress.com/Demos/StockMarket/odata/StockDatas" +
        options.method,
      dataType: "json",
  ...
  var StockMarketView = function () {
      ...
      onSelectionChanged: function (items) {
        if (items.selectedRowsData[0]) {
          ...
          if (items.currentDeselectedRowKeys.length) {
            grid
              .getCellElement(
                grid.getRowIndexByKey(items.currentDeselectedRowKeys[0]),
                "Volume Dynamics"
              )
              .children()
              .dxSparkline({lineColor: "#d01143"});
            grid
              .getCellElement(
                grid.getRowIndexByKey(items.currentSelectedRowKeys[0]),
                "Volume Dynamics"
              )
              .children()
              .dxSparkline({lineColor: "#fff"});
          }
                ...
                $.each(data, function (_, item) {
                  $.each(categories, function (_, category) {
                    for (var key in category) {
                      if (
                        $.inArray(item.Company.CompanyName, category[key]) >= 0
                      )
    ...
    var tmpMenuOptions = {
      ...
      onItemClick: function (data) {
        if (!data.itemData.items) {
          that.weeksIntervalSelectedItem = data.itemData;
      ...
      onSelectionChanged: function (data) {
        if (!data.addedItems[0] && that.weeksIntervalSelectedItem) {
          setTimeout(function () {
            data.component.selectItem(that.weeksIntervalSelectedItem);
          }, 0);
        }
      },
    ...
    that.weeksIntervalSelectedItem = null;
    ...
    that.updateData = function () {
      ...
      function getData() {
        ...
        if (i === 0) {
          that.updatePivotGridData();
        }

        StockMarket.loadData({
          method:
            "?$expand=Company&$filter= Date le datetime'" +
            that.currentDate +
            "'&$orderby=Date desc&$top=320",
              ...
              $.each(data.value, function (_, item1) {
                ...
                $.each(data.value, function (_, item2) {
                  if (item1.CompanyID == item2.CompanyID)
                    item1.sparkLineData.push({
                      Date: item2.Date,
                      Volumne: item2.Volumne
                    });
                });
              });
    ...
    that.initCharts = function (selectItemData) {
      ...
      StockMarket.loadData({
        method:
          "?$expand=Company&$filter=CompanyID eq " +
          selectItemData.CompanyID +
          " and Date ge datetime'" +
          startDate +
          "' and Date le datetime'" +
          that.currentDate +
          "'",
        callback: function (data) {
          ...
          if (oldData.length) {
            ...
            for (i = oldData.length - 1; i > 0; i--) {
              ...
              if (condition) newData.push(oldData[i]);
            }
    ...
    that.updatePivotGridData = function () {
      ...
      StockMarket.loadData({
        method:
          "?$expand=Company&$select=Date,Price,Company/CompanyName&$filter=Date ge datetime'" +
          startDate +
          "' and Date le datetime'" +
          that.currentDate +
          "'&$orderby=Date desc&$top=9000",
        callback: function (data) {
    ...
    that.initChart = function (options, type) {
      ...
      if (type == "pie")
        ...
        chart = chartContainer
          .dxChart($.extend(true, {}, options, commonOptions))
          .dxChart("instance");
  ...
  $(function () {
    ...
    var changeIntervalAction = function (e) {
        StockMarket.StockMarketView.changeInterval(e);
      },
      redrawViewsAction = function (e) {
        StockMarket.StockMarketView.redrawViews(e);
      },
      buttonsArray = [
        {
          CSSClass: "button-charts",
          clickAction: redrawViewsAction,
          width: 54,
          height: 42
        },
        {
          CSSClass: "button-pivotgrid-chart",
          clickAction: redrawViewsAction,
          width: 54,
          height: 42
        },
        {
          ...
          clickAction: function (e) {
            StockMarket.StockMarketView.initView("pivotgrid-chart", true);
          }
        },
