          ...
          <v-row class="d-flex justify-start align-center">
            ...
            <v-col cols="12">
              <div class="search-item rounded-lg">
                <MultiSelectBox
                  class="detail-search-item-divider"
                  :items="this.commonData.facilitiesName"
                  icon="icon-order-number.png"
                  :title="$t(`wcs.exec.execMachStatus.machNm`)"
                  @keyup.enter="save"
                  @selected="this.filterQuery.facilitiesName = $event">
                </MultiSelectBox>
              </div>
            </v-col>
            <!-- 설비명 -->
...
export default {
  ...
  created() {
    ...
    const facilitiesName = [
      {text: "3D-Sorter-01호기", value: "3D-Sorter-01호기"},
      {text: "3D-Sorter-02호기", value: "3D-Sorter-02호기"},
      {text: "3D-Sorter-03호기", value: "3D-Sorter-03호기"}
    ];
    this.commonData.facilitiesName = facilitiesName;
  },
  mounted() {},
  methods: {
    /**************************************************************************
    ...
    async save() {
      ...
      if (this.filterQuery.facilitiesName.length > 0) {
        for (let j = 0; j < this.filterQuery.facilitiesName.length; j++) {
          lktBody.push({
            workDate: dayjs(this.filterQuery.workDate).format(format),
            facilitiesName: this.filterQuery.facilitiesName[j]
          });
        }
      } else {
        lktBody.push({
